...
const {createSocketIo} = require("./black-socket");

var WebSocketServer = require("ws").Server;

var wsserver = new WebSocketServer({port: 28374});
...
io
    .on('connection', function (socket) {
        ...
        socket.on("create", function ({
            ...
            getEditor(language, content)
                .then(function ({editor, TextDocument}) {
...
function activate(context) {
    ...
    var disposable = vscode
        .commands
        .registerCommand('extension.linkWithLeetCode', function () {
            vscode
                .window
                .showInformationMessage(`link-with-leetcode's server is running, click 'Open in VS Code button to start linking'`);
            // vscode     .window     .setStatusBarMessage(`link-with-leetcode's server is
            // running, click 'Open in VS Code button to start linking'`, 10000)
        });
